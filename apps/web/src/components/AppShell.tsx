    ...
    <div className="min-h-screen lg:grid lg:grid-cols-[248px_1fr]">
      <aside className="border-b bg-ink text-white lg:min-h-screen lg:border-b-0 lg:border-r lg:border-white/10">
        <div className="flex h-16 items-center gap-3 border-b border-white/10 px-5">
          <span className="grid h-8 w-8 place-items-center rounded-lg bg-emerald-400/15"><Activity className="h-4 w-4 text-emerald-300" /></span>
          <div><div className="text-sm font-semibold tracking-tight">SupplySignal</div><div className="text-[10px] uppercase tracking-[0.18em] text-white/45">Operations intelligence</div></div>
        </div>
        <nav className="flex gap-1 overflow-x-auto p-3 lg:block lg:space-y-1" aria-label="Primary navigation">
          {navigation.map(({ label, to, icon: Icon }) => (
            <NavLink key={to} to={to} className={({ isActive }) => `focus-ring flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${isActive ? 'bg-white/10 text-white' : 'text-white/60 hover:bg-white/5 hover:text-white'}`}>
              <Icon className="h-4 w-4" aria-hidden="true" />{label}
            </NavLink>
          ))}
        </nav>
        <div className="hidden px-3 lg:absolute lg:bottom-4 lg:block lg:w-[248px]">
          <button className="focus-ring flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-white/60 hover:bg-white/5 hover:text-white" onClick={() => void supabase?.auth.signOut()}><LogOut className="h-4 w-4" /> Sign out</button>
        </div>
      </aside>
