...
type RelevantArticle = {
  ...
  discoveredAt: string; category: string; source: { name: string; status: string };
  relatedSuppliers: string[]; relatedFactories: string[]; relatedProducts: string[];
  relatedMaterials: string[]; reasons: string[];
};
type PocSource = {
  id: string; name: string; type: string; url: string; category: string;
  lastChecked: string | null; status: string;
...
function ArticleCard({ article }: { article: RelevantArticle }) {
  const related = [...article.relatedSuppliers, ...article.relatedFactories, ...article.relatedProducts, ...article.relatedMaterials];
  return <article className="rounded-lg border p-4"><div className="flex flex-wrap gap-2 text-[11px] uppercase tracking-wide text-muted"><span>{article.category}</span><span>·</span><span>{article.source.name}</span><span>·</span><span>{article.publishedAt ? new Date(article.publishedAt).toLocaleDateString() : 'Date unavailable'}</span></div><a className="mt-2 block font-semibold text-signal hover:underline" href={article.url} target="_blank" rel="noreferrer">{article.title}</a>{article.summary && <p className="mt-2 line-clamp-3 text-sm text-muted">{article.summary}</p>}<div className="mt-3 text-xs"><span className="font-semibold">Related BSK data:</span> <span className="text-muted">{related.join(', ') || 'Location match'}</span></div><p className="mt-1 text-xs text-muted">{article.reasons[0]}</p></article>;
}
