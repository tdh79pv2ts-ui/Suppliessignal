...
export class SupplyChainService {
  ...
  async graph(customerId: string) {
    ...
    const suppliers = await db.supplier.findMany({ where: { customerId } });
    const factories = await db.factory.findMany({ where: { customerId } });
    const products = await db.product.findMany({ where: { customerId } });
    const materials = await db.material.findMany({ where: { customerId } });
    const routes = await db.route.findMany({ where: { customerId } });
    const companySuppliers = await db.companySupplier.findMany({ where: { customerId } });
    const factoryLocations = await db.factoryLocation.findMany({ where: { customerId } });
    const supplierProducts = await db.supplierProduct.findMany({ where: { customerId } });
    const factoryProducts = await db.factoryProduct.findMany({ where: { customerId } });
    const productMaterials = await db.productMaterial.findMany({ where: { customerId } });
    const routeSuppliers = await db.routeSupplier.findMany({ where: { customerId } });
    const routeFactories = await db.routeFactory.findMany({ where: { customerId } });
    const routePorts = await db.routePort.findMany({ where: { customerId }, orderBy: [{ routeId: 'asc' }, { sequence: 'asc' }] });
