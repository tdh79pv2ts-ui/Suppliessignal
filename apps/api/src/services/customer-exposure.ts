...
export class CustomerExposureService {
  async reconcileEvent(eventId: string) {
    ...
    return db.$transaction(async (tx) => {
      ...
      for (const entity of event.entities) {
        for (const identifier of entity.identifiers.filter((item) => item.verificationStatus === 'VERIFIED')) {
          ...
          for (const identity of identities) {
            ...
            findings.push({ customerId: identity.customerId, exposureType, matchState: 'VERIFIED_DIRECT', method: node.nodeType === 'PORT' ? 'GLOBAL_PORT_ID' : 'VERIFIED_IDENTIFIER', confidence: 1, reason: node.nodeType === 'PORT' ? 'EXACT_PORT_ID' : `EXACT_${node.nodeType}_IDENTIFIER` as Finding['reason'], eventEntityIds: [entity.id], eventLocationIds: [], nodes: [node] });
            if (node.nodeType === 'PORT') {
              const links = await tx.routePort.findMany({ where: { portId: node.id, route: { active: true } }, include: { route: true } });
      ...
      for (const exposure of existing.filter((item) => !grouped.has(item.customerId))) {
        await tx.customerExposure.update({ where: { id: exposure.id }, data: event.status === 'RESOLVED' || event.status === 'CANCELLED' ? { status: 'RESOLVED', resolvedAt: new Date() } : { status: 'STALE', staleAt: new Date() } });
        await tx.exposurePath.updateMany({ where: { exposureId: exposure.id, activeMatch: true }, data: { activeMatch: false, invalidatedAt: new Date() } });
      }
  ...
  async reviewCandidate(customerId: string, id: string, userId: string, confirmed: boolean, reasonCode: 'AMBIGUOUS_ENTITY_IDENTITY' | 'AMBIGUOUS_LOCATION' | 'NO_SUPPORTED_MATCH', resultingIdentityId?: string) { const candidate = await this.getCandidate(customerId, id); if (confirmed && !resultingIdentityId) throw new ServiceError('IDENTITY_REQUIRED', 'Confirmation requires a customer graph identity', 400); if (confirmed && resultingIdentityId) { const identity = await db.customerGraphIdentity.findFirst({ where: { id: resultingIdentityId, customerId, verificationStatus: 'VERIFIED' } }); if (!identity) throw new ServiceError('VERIFIED_IDENTITY_REQUIRED', 'Confirmation requires a verified identity for this customer', 409); const subjectId = identity.supplierId ?? identity.factoryId ?? identity.productId ?? identity.materialId ?? identity.routeId ?? identity.portId; const candidateOwnsSubject = candidate.nodes.some((node) => (node.supplierId ?? node.factoryId ?? node.productId ?? node.materialId ?? node.routeId ?? node.portId) === subjectId); if (!candidateOwnsSubject) throw new ServiceError('IDENTITY_NOT_IN_CANDIDATE', 'The resulting identity must represent a candidate node', 409); } const item = await db.exposureCandidate.update({ where: { id }, data: { status: confirmed ? 'CONFIRMED' : 'REJECTED', reviewedAt: new Date(), reviewedByUserId: userId, reviewReasonCode: reasonCode, ...(resultingIdentityId ? { resultingIdentityId } : {}) } }); if (confirmed) await this.reconcileEvent(item.eventId); return item; }
  ...
  async createAdminEventIdentity(eventEntityId: string, userId: string, input: Omit<EventIdentityProposalInput, 'eventEntityId'>) { const entity = await db.eventEntity.findUnique({ where: { id: eventEntityId } }); if (!entity) throw new ServiceError('EVENT_ENTITY_NOT_FOUND', 'Event entity not found', 404); if (input.sourceClaimId && !await db.eventClaim.findUnique({ where: { eventId_claimId: { eventId: entity.eventId, claimId: input.sourceClaimId } } })) throw new ServiceError('CLAIM_NOT_IN_EVENT', 'Source Claim does not support this Event', 409); const created = await db.$transaction(async (tx) => { const identifier = await tx.eventEntityIdentifier.create({ data: { eventEntityId, namespace: input.namespace, identifier: input.identifier, normalizedIdentifier: normalizeIdentifier(input.identifier), verificationStatus: 'VERIFIED', provenanceSource: input.provenanceSource, provenanceRef: input.provenanceRef ?? null, evidenceNote: input.evidenceNote ?? null, sourceClaimId: input.sourceClaimId ?? null, verifiedAt: new Date(), verifiedByUserId: userId } }); await tx.event.update({ where: { id: entity.eventId }, data: { exposureVersion: { increment: 1 } } }); return identifier; }); await this.reconcileEvent(entity.eventId); return created; }
