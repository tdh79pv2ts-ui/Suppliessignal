...
The five-minute POC worker checks only RSS/Atom sources enabled by at least one customer preference, preserves each publisher URL, translates before matching when translation is enabled, and then applies policy-versioned deterministic relevance. Direct verified graph matches become Direct impact, explicit location/country dependencies become Potential impact, and processed material supply-chain signals without a direct match can appear as Broader developments with an explicit uncertainty statement. A source's country is discovery metadata, not proof that every article concerns that country.
