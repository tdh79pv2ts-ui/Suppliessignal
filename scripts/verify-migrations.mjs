...
await withDisposablePostgres(async ({ env, bin, port, databaseName }) => {
  ...
  run(
    ...
    [
      ...
      'ON_ERROR_STOP=1',
      '-c',
      "SELECT (SELECT count(*) FROM customers) AS customers, (SELECT count(*) FROM suppliers) AS suppliers, (SELECT count(*) FROM route_ports) AS route_ports, (SELECT count(*) FROM sources) AS sources, (SELECT count(*) FROM sources WHERE collection_enabled) AS collection_enabled_sources, to_regclass('public.customer_exposures') IS NOT NULL AS exposures_available, to_regclass('public.exposure_path_steps') IS NOT NULL AS path_steps_available, to_regclass('public.exposure_candidates') IS NOT NULL AS candidates_available, to_regclass('public.event_entity_identifiers') IS NOT NULL AS event_identifiers_available;",
    ],
    { env },
  );
});
