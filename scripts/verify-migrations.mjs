...
await withDisposablePostgres(async ({ env, bin, port, databaseName }) => {
  ...
  run(
    ...
    [
      ...
      'ON_ERROR_STOP=1',
      '-c',
      "DO $$ BEGIN IF (SELECT count(*) FROM companies WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b') <> 6 OR (SELECT count(*) FROM suppliers WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b' AND source_url IS NOT NULL) <> 5 OR (SELECT count(*) FROM factories WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b' AND source_url IS NOT NULL) <> 3 OR (SELECT count(*) FROM products WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b' AND source_url IS NOT NULL) <> 1 OR (SELECT count(*) FROM materials WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b' AND source_url IS NOT NULL) <> 6 OR (SELECT count(*) FROM routes WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b') <> 0 OR (SELECT count(*) FROM company_suppliers WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b' AND source_url IS NOT NULL) <> 5 OR EXISTS (SELECT 1 FROM suppliers WHERE supplier_type='FICTIONAL_DEMO') OR (SELECT count(*) FROM sources WHERE id IN ('a1000000-0000-4000-8000-000000000001','a1000000-0000-4000-8000-000000000002','a2000000-0000-4000-8000-000000000001','a2000000-0000-4000-8000-000000000002','a2000000-0000-4000-8000-000000000003') AND collection_enabled AND collection_interval_minutes=5 AND feed_url IS NOT NULL) <> 5 OR (SELECT count(*) FROM factories WHERE customer_id='b5000000-0000-4000-8000-000000000001' AND source_url IS NOT NULL) <> 4 OR (SELECT count(*) FROM products WHERE customer_id='b5000000-0000-4000-8000-000000000001' AND source_url IS NOT NULL) <> 6 OR (SELECT count(*) FROM materials WHERE customer_id='b5000000-0000-4000-8000-000000000001' AND source_url IS NOT NULL) <> 5 OR (SELECT count(*) FROM locations WHERE customer_id='b5000000-0000-4000-8000-000000000001' AND source_url IS NOT NULL) <> 4 OR (SELECT count(*) FROM suppliers WHERE customer_id='b5000000-0000-4000-8000-000000000001') <> 0 OR EXISTS (SELECT 1 FROM source_articles WHERE original_url LIKE '%.invalid%') THEN RAISE EXCEPTION 'Evidence-backed POC seed or real-source configuration is invalid'; END IF; END $$; SELECT (SELECT count(*) FROM customers) AS customers, (SELECT count(*) FROM companies WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b') AS poc_companies, (SELECT count(*) FROM suppliers WHERE customer_id='5a6ce6b4-0d65-4d16-90dc-04b751f5169b') AS poc_suppliers, (SELECT count(*) FROM factories WHERE customer_id='b5000000-0000-4000-8000-000000000001') AS bsk_factories, (SELECT count(*) FROM sources WHERE collection_enabled) AS collection_enabled_sources, to_regclass('public.daily_briefs') IS NOT NULL AS briefs_available;",
    ],
    { env },
  );
});
