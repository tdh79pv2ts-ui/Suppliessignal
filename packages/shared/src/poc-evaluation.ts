...
export const pocThresholdsSchema = z.object({
  evidenceAccuracy: z.number().min(0).max(1),
  unsupportedRate: z.number().min(0).max(1),
  claimCorrectRate: z.number().min(0).max(1),
  entityAccuracy: z.number().min(0).max(1),
  locationAccuracy: z.number().min(0).max(1),
  dateAccuracy: z.number().min(0).max(1),
  assertionAccuracy: z.number().min(0).max(1),
  extractionFailureRate: z.number().min(0).max(1),
  relevancePrecision: z.number().min(0).max(1),
  relevanceRecall: z.number().min(0).max(1),
}).strict();
...
export type PocThresholds = z.infer<typeof pocThresholdsSchema>;
...
function distribution(scores: EvaluationScoreValue[]) {
  const applicable = scores.filter((score) => score !== 'NOT_APPLICABLE');
  const total = applicable.length;
  return {
    correct: ratio(applicable.filter((score) => score === 'CORRECT').length, total),
    partial: ratio(applicable.filter((score) => score === 'PARTIALLY_CORRECT').length, total),
    incorrect: ratio(applicable.filter((score) => score === 'INCORRECT').length, total),
    blended: total ? applicable.reduce((sum, score) => sum + (score === 'CORRECT' ? 1 : score === 'PARTIALLY_CORRECT' ? 0.5 : 0), 0) / total : null,
  };
}
...
export function calculatePocResults(input: PocMetricInput) {
  ...
  const claim = distribution(score('claimScore'));
  const evidence = distribution(score('evidenceScore'));
  const entity = distribution(score('entityScore'));
  const location = distribution(score('locationScore'));
  const date = distribution(score('dateScore'));
  const assertion = distribution(score('assertionScore'));
  const metrics = {
    relevancePrecision: ratio(tp, tp + fp), relevanceRecall: ratio(tp, tp + fn),
    claimCorrectRate: claim.correct, claimPartialRate: claim.partial, claimIncorrectRate: claim.incorrect,
    unsupportedRate: ratio(reviewedClaims.filter((c) => c.evaluation!.unsupported).length, reviewedClaims.length),
    evidenceAccuracy: evidence.correct, evidencePartialRate: evidence.partial, evidenceIncorrectRate: evidence.incorrect, evidenceBlendedRate: evidence.blended,
    entityAccuracy: entity.correct, entityPartialRate: entity.partial, entityIncorrectRate: entity.incorrect, entityBlendedRate: entity.blended,
    locationAccuracy: location.correct, locationPartialRate: location.partial, locationIncorrectRate: location.incorrect, locationBlendedRate: location.blended,
    dateAccuracy: date.correct, datePartialRate: date.partial, dateIncorrectRate: date.incorrect, dateBlendedRate: date.blended,
    assertionAccuracy: assertion.correct, assertionPartialRate: assertion.partial, assertionIncorrectRate: assertion.incorrect, assertionBlendedRate: assertion.blended,
  ...
  const checks = {
    evidenceAccuracy: metrics.evidenceAccuracy !== null && metrics.evidenceAccuracy >= input.thresholds.evidenceAccuracy,
    unsupportedRate: metrics.unsupportedRate !== null && metrics.unsupportedRate <= input.thresholds.unsupportedRate,
    claimCorrectRate: metrics.claimCorrectRate !== null && metrics.claimCorrectRate >= input.thresholds.claimCorrectRate,
    entityAccuracy: metrics.entityAccuracy !== null && metrics.entityAccuracy >= input.thresholds.entityAccuracy,
    locationAccuracy: metrics.locationAccuracy !== null && metrics.locationAccuracy >= input.thresholds.locationAccuracy,
    dateAccuracy: metrics.dateAccuracy !== null && metrics.dateAccuracy >= input.thresholds.dateAccuracy,
    assertionAccuracy: metrics.assertionAccuracy !== null && metrics.assertionAccuracy >= input.thresholds.assertionAccuracy,
    extractionFailureRate: metrics.extractionFailureRate !== null && metrics.extractionFailureRate <= input.thresholds.extractionFailureRate,
    relevancePrecision: metrics.relevancePrecision !== null && metrics.relevancePrecision >= input.thresholds.relevancePrecision,
    relevanceRecall: metrics.relevanceRecall !== null && metrics.relevanceRecall >= input.thresholds.relevanceRecall,
  };
