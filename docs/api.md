...
Membership-scoped reads are available at `/api/customers/:customerId/news-radar`, `/monitoring-profile`, `/intelligence`, `/exposures`, and `/exposures/:exposureId`. They expose only deterministic Article-to-graph intelligence for the requested customer. ADMIN may process a single article with `POST /api/admin/news-radar/articles/:articleId/process`, a bounded pending batch with `POST /api/admin/news-radar/process-pending`, inspect durable lifecycle evidence with `GET /api/admin/poc-ingestion/status`, or explicitly run an initial/delta/reconciliation cycle with `POST /api/admin/poc-ingestion/run`. These routes do not use Claims, Events, identity resolution, risk scoring, or review decisions.
