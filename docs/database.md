...
Phase 5 adds global `events`, `event_claims`, `event_entities`, `event_locations`, and `claim_event_processing`. EventClaim is an explicit many-to-many provenance link, so reprocessing history remains inspectable. Conservative normalized keys and unique constraints prevent duplicate entity/location rows per Event. A unique fingerprint, recoverable Claim processing lease, and transaction advisory lock make event creation idempotent and safe across API/worker processes. No Phase 5 table carries or infers a customer relationship.
