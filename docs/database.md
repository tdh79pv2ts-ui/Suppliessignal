...
Create local migrations with `pnpm db:migrate`; apply committed migrations to staging/production with `pnpm db:deploy`. Seed explicitly with `pnpm db:seed`, and inspect locally with `pnpm db:studio`. The seed customer and user are fictional. No password is stored in PostgreSQL.
...
Phase 5 adds global `events`, `event_claims`, `event_entities`, `event_locations`, and `claim_event_processing`. EventClaim is an explicit many-to-many provenance link with a deterministic ClaimSignal, so reprocessing and resolution/cancellation evidence remain inspectable. Events persist their construction policy version; migrations retain `1.0` for historic Phase 5 rows. Conservative normalized keys and unique constraints prevent duplicate entity/location rows per Event. A unique versioned fingerprint, recoverable Claim processing lease, and transaction advisory lock make event creation idempotent and safe across API/worker processes. No Phase 5 table carries or infers a customer relationship.
