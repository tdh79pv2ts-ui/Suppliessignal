...
`pnpm verify:start` uses another disposable database and production-like staging configuration to verify Railway `PORT` precedence, API health, and disabled worker startup without an OpenAI key.
